# DISTRIBUTED ANYCAST GATEWAY (DAG) USE CASE #1
...
### Per node variables for _leafs_ nodes:
  ...
  * **Example**: two leaf nodes, one simple "leaf", one "border" leaf:
    ...
    ```
      ...
      * Note that common variables like **ip_mask** or **dag_ipv4_mask** can be removed from the individual nodes and moved to the "vars" section of the group.
...
### Group variables for "network" group (whole network, all nodes):
  ...
  * **Example**: "network" group, conformed by _leafs_ and _spines_ groups as "children" plus all the variables needed by this group under the section "vars":
    ...
    ```
     * Note the indentation for YAML (two spaces).
     * Note that common variables like **ip_mask** or **dag_ipv4_mask** can be removed from the individual nodes and moved to the "vars" section for the whole network under the group "network", as long as the value of this variables is the same.
...
### Inventory example:
  * A full inventory, three leafs, two simple leafs, one border leaf and two spines, with all the needed variables looks like this:
    ...
    ```
      * Note the indentation for YAML (two spaces).
      * Note that common variables like **ip_mask** or **dag_ipv4_mask** can be removed from the individual nodes and moved to the "vars" section of the group or to the "vars" section for the whole network under the group "network", as long as the value of this variables is the same.
...
### Main playbook execution example:
  ...
  * **The main playbook execution should look like this**:
    ...
    TASK [dag : CONFIG L2VNI EDGE L2 VLAN] ***************************************************************
    ok: [sw-access-leaf-2]
    ok: [sw-access-leaf-1]

    TASK [dag : CONFIG L2VNI EDGE VLAN CONFIGURATION MEMBER] *********************************************
    ok: [sw-access-leaf-2]
    ok: [sw-access-leaf-1]
    ...
    TASK [dag : CONFIG L2VNI EDGE SVI VLAN INTERFACE] ****************************************************
    changed: [sw-access-leaf-2]
    changed: [sw-access-leaf-1]
    ...
    TASK [dag : CONFIG L2VNI INTERFACE EDGE VLAN SVI] ****************************************************
    changed: [sw-access-leaf-2]
    changed: [sw-access-leaf-1]
    ...
    TASK [dag : CONFIG L2 CORE VLAN MEMBER L3 VNI CONFIGURATION] *****************************************
    changed: [sw-access-leaf-2]
    changed: [sw-access-leaf-1]

    TASK [dag : CONFIG L3 SVI CORE VLAN INTERFACE] *******************************************************
    changed: [sw-access-leaf-2]
    changed: [sw-access-leaf-1]
